function [element, document] = serialize(obj, tagName, document, parentElement)
    ...
    if isenum(obj) && length(obj) < 2
        % currently doesn't support enum arrays
        value = char(obj);
        valueNode = document.createTextNode(value);
        element.appendChild(valueNode);
    elseif ~isenum(obj) && mxml.legacy.private.isPrimitiveValueType(obj)
        if isnumeric(obj) || islogical(obj)
            value = mxml.legacy.private.convertMat2str(obj);
        elseif ischar(obj)
            value = mxml.legacy.private.charmat2str(obj);
        end
        try
        valueNode = document.createTextNode(value);
        element.appendChild(valueNode);
    ...
    elseif (isvector(obj) && length(obj) > 1) || iscell(obj)
        ...
        for i = 1:length(obj)
            mxml.legacy.private.serialize(mxml.legacy.private.accessArray(obj, i), 'entry', document, element);
        end
    ...
    elseif ~isempty(obj)
        ...
        for i = 1:length(fields)
            ...
            mxml.legacy.private.serialize(fieldValue, fieldName, document, element);
        end
    end
end
